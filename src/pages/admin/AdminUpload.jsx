      ...
      <div className="bg-white rounded-xl shadow-sm border border-[#EADBC8]/30 p-6">
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          {selectedFiles.length > 0 && (
            <div>
              <h3 className="text-lg font-medium text-[#102C57] mb-3">
      ...
      {showModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
          <div className="bg-white rounded-lg shadow-lg p-6 w-full max-w-sm mx-auto text-center">
            <h2 className="text-xl font-bold text-[#102C57] mb-2">{modalContent.title}</h2>
            <p className="text-[#102C57]/80 mb-4">{modalContent.message}</p>
            <button
              onClick={() => setShowModal(false)}
              className="mt-2 px-4 py-2 bg-[#102C57] text-white rounded hover:bg-[#102C57]/90 transition-colors"
            >
              Close
            </button>
          </div>
